...
def G():
    ...
    background = pg.image.load('Z:/data/background.png')
    background_rect = background.get_rect(bottomright=[SIZE, SIZE])

    font = pg.font.Font('Z:/data/gogono-cocoa-mochi-cyrillic.otf', 28)
    fontchat = pg.font.Font('Z:/data/gogono-cocoa-mochi-cyrillic.otf', 23)
...
def registration():
    ...
    tg = pg.image.load('Z:/data/tg.png')
...
def sql():
    with sqlite3.connect("Z:/data/data.db") as db:
        cursor = db.cursor()
        cursor.execute(
            """
...
def save_user(nickname, password, registration_surface):
    db = sqlite3.connect('Z:/data/data.db')
    cursor = db.cursor()
...
def game(nickname, game_surface, size):
    ...
    donate = pg.image.load("Z:/data/donate.png")
